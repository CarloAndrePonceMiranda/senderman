...
keep = {".git", ".venv", ".env", "panel.log", "users.json", "senderman_registry.sqlite3", "backups", "local-tools"}
...
if [ "${SENDERMAN_INSTALLER_SKIP_BOOTSTRAP:-0}" != "1" ]; then
  ...
  release_info="$(select_release_info "$repo_slug" "$release_mode" "$release_selector")"
